...
This explains the details for training Focal Transformers on image classification. We have the same instructions to [Swin Transfomer](https://github.com/microsoft/Swin-Transformer)
...
To evaluate a pre-trained `Focal Transformer` on ImageNet val, run:
...
For example, to evaluate the `Focal-Base` with a single GPU:

```bash
python -m torch.distributed.launch --nproc_per_node 1 --master_port 12345 main.py --eval \
--cfg configs/diagswin_base_patch4_window7_224.yaml --resume focal-base-is224-ws7.pth --data-path <imagenet-path>
```

### Training from scratch

To train a `Focal Transformer` on ImageNet from scratch, run:
...
For example, to train `Focal Transformer` with 8 GPU on a single node for 300 epochs, run:

`Focal-Tiny`:

```bash
python -m torch.distributed.launch --nproc_per_node 8 --master_port 12345  main.py \
--cfg configs/diagswin_tiny_patch4_window7_224.yaml --data-path <imagenet-path> --batch-size 128 
```
...
```bash
python -m torch.distributed.launch --nproc_per_node 8 --master_port 12345  main.py \
--cfg configs/diagswin_small_patch4_window7_224.yaml --data-path <imagenet-path> --batch-size 128 
```
...
```bash
python -m torch.distributed.launch --nproc_per_node 8 --master_port 12345  main.py \
--cfg configs/diagswin_base_patch4_window7_224.yaml --data-path <imagenet-path> --batch-size 64 \
