...
class WindowAttention(nn.Module):
    ...
    Args:
        ...
        expand_size (int): The expand size at diagswin level 1.
        window_size (tuple[int]): The height and width of the window.
        diagswin_window (int): DiagSWin region size.
        diagswin_level (int): DiagSWin attention level.
    ...
    def __init__(self, dim, expand_size, window_size, diagswin_window, diagswin_level, num_heads,
        ...
        self.diagswin_level = diagswin_level
        self.diagswin_window = diagswin_window
        ...
        if self.expand_size > 0 and diagswin_level > 0:
            ...
            self.window_size_of_key = self.window_size[0] * self.window_size[1] if self.expand_size == 0 else \
                ((self.window_size[0] + 2 * self.expand_size) * (self.window_size[1] + 2 * self.expand_size) - (
                            self.window_size[0] * self.window_size[1]) - 2 * (self.expand_size * self.expand_size))
            ...
            # use_shift = False: Left Diagonal Sub-window
            # use_shift = True: Right Diagonal Sub-window
            if self.use_shift == False:
                # get mask for rolled k and rolled v
                mask_tl = torch.zeros(self.window_size[0], self.window_size[1]);
                mask_tl[-self.expand_size:, -self.expand_size:] = 1
                mask_br = torch.zeros(self.window_size[0], self.window_size[1]);
                mask_br[:self.expand_size, :self.expand_size] = 1
                mask_t = torch.ones(self.window_size[0], self.window_size[1]);
                mask_t[:-self.expand_size, :] = 0
                mask_b = torch.ones(self.window_size[0], self.window_size[1]);
                mask_b[self.expand_size:, :] = 0
                mask_l = torch.ones(self.window_size[0], self.window_size[1]);
                mask_l[:, :-self.expand_size] = 0
                mask_r = torch.ones(self.window_size[0], self.window_size[1]);
                mask_r[:, self.expand_size:] = 0
                mask_rolled = torch.stack((mask_tl, mask_br, mask_t, mask_b, mask_l, mask_r), 0).flatten(0)
            else:
                # get mask for rolled k and rolled v
                mask_tr = torch.zeros(self.window_size[0], self.window_size[1]);
                mask_tr[-self.expand_size:, :self.expand_size] = 1
                mask_bl = torch.zeros(self.window_size[0], self.window_size[1]);
                mask_bl[:self.expand_size, -self.expand_size:] = 1
                mask_t = torch.ones(self.window_size[0], self.window_size[1]);
                mask_t[:-self.expand_size, :] = 0
                mask_b = torch.ones(self.window_size[0], self.window_size[1]);
                mask_b[self.expand_size:, :] = 0
                ...
                mask_r = torch.ones(self.window_size[0], self.window_size[1]);
                mask_r[:, self.expand_size:] = 0
                mask_rolled = torch.stack((mask_tr, mask_bl, mask_t, mask_b, mask_l, mask_r), 0).flatten(0)
            self.register_buffer("valid_ind_rolled", mask_rolled.nonzero().view(-1))

        if pool_method != "none" and diagswin_level > 1:
            ...
            for k in range(diagswin_level-1):
                stride = 2**k    
                kernel_size = 2*(self.diagswin_window // 2) + 2**k + (2**k-1)
                ...
                relative_position_bias_table_to_windows = nn.Parameter(
                    torch.zeros(
                        self.num_heads,
                        (self.window_size[0] + self.diagswin_window + 2**k - 2) * (self.window_size[1] + self.diagswin_window + 2**k - 2),
                        )
                )
                ...
                relative_position_index_k = get_relative_position_index(self.window_size, to_2tuple(self.diagswin_window + 2**k - 1))
                self.register_buffer("relative_position_index_{}".format(k), relative_position_index_k)

                # define unfolding index for diagswin_level > 0
                if k > 0:
    ...
    def forward(self, x_all, mask_all=None):
        ...
        if self.expand_size > 0 and self.diagswin_level > 0:
            (k_t, v_t) = map(
                lambda t: torch.roll(t, shifts=(-self.expand_size, 0), dims=(1, 2)), (k, v)
            )
            (k_b, v_b) = map(
                lambda t: torch.roll(t, shifts=(self.expand_size, 0), dims=(1, 2)), (k, v)
            )
            (k_l, v_l) = map(
                lambda t: torch.roll(t, shifts=(0, -self.expand_size), dims=(1, 2)), (k, v)
            )
            (k_r, v_r) = map(
                lambda t: torch.roll(t, shifts=(0, self.expand_size), dims=(1, 2)), (k, v)
            )
            # use_shift = False: Left Diagonal Sub-window
            # use_shift = True: Right Diagonal Sub-window
            # For the convenience of unifying the name: td->Top Diagonal, bd->Bottom Diagonal
            if self.use_shift == False:
                (k_td, v_td) = map(
            ...
            (k_td_windows, k_bd_windows, k_t_windows, k_b_windows, k_l_windows, k_r_windows) = map(
                ...
                (k_td, k_bd, k_t, k_b, k_l, k_r)
            )
            (v_td_windows, v_bd_windows, v_t_windows, v_b_windows, v_l_windows, v_r_windows) = map(
                lambda t: window_partition(t, self.window_size[0]).view(-1, self.window_size[0] * self.window_size[0],
                                                                        self.num_heads, C // self.num_heads),
                (v_td, v_bd, v_t, v_b, v_l, v_r)
            )

            k_rolled = torch.cat((k_td_windows, k_bd_windows, k_t_windows, k_b_windows, k_l_windows, k_r_windows),
                                 1).transpose(1, 2)
            v_rolled = torch.cat((v_td_windows, v_bd_windows, v_t_windows, v_b_windows, v_l_windows, v_r_windows),
                                 1).transpose(1, 2)
        ...
        if self.pool_method != "none" and self.diagswin_level > 1:
            k_pooled = []
            v_pooled = []
            for k in range(self.diagswin_level-1):
                stride = 2**k
        ...
        attn = (q_windows @ k_all.transpose(-2, -1))  # B*nW, nHead, window_size*window_size, diagswin_window_size * diagswin_window_size
        ...
        if self.pool_method != "none" and self.diagswin_level > 1:
            ...
            for k in range(self.diagswin_level-1):
                ...
                relative_position_bias_to_windows = self.relative_position_bias_table_to_windows[k][:, relative_position_index_k.view(-1)].view(
                    -1, self.window_size[0] * self.window_size[1], (self.diagswin_window+2**k-1)**2,
                ) # nH, NWh*NWw, diagswin_region * diagswin_region
                attn[:, :, :window_area, offset:(offset + (self.diagswin_window+2**k-1)**2)] = \
                    attn[:, :, :window_area, offset:(offset + (self.diagswin_window+2**k-1)**2)] + relative_position_bias_to_windows.unsqueeze(0)
                ...
                offset += (self.diagswin_window+2**k-1)**2
...
class DiagSWinTransformerBlock(nn.Module):
    r""" DiagSWin Transformer Block.
    ...
    Args:
        ...
        num_heads (int): Number of attention heads.
        window_size (int): Window size.
        expand_size (int): expand size at first diagswin level (finest level).
        ...
        diagswin_level (int): number of diagswin levels. Default: 1.
        diagswin_window (int): region size of diagswin attention. Default: 1
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, expand_size=0, use_shift=False,
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, pool_method="none", 
                 diagswin_level=1, diagswin_window=1, use_layerscale=False, layerscale_value=1e-4):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            diagswin_window=diagswin_window, diagswin_level=diagswin_level, num_heads=num_heads,
    ...
    def forward(self, x):
        ...
        if self.diagswin_level > 1 and self.pool_method != "none":
    ...
    def flops(self):
        ...
        flops += nW * self.attn.flops(self.window_size * self.window_size, self.window_size, self.diagswin_window)
...
class BasicLayer(nn.Module):
    """ A basic diagswin Transformer layer for one stage.
    ...
    Args:
        ...
        expand_size (int): expand size for diagswin level 1.
        ...
        pool_method (str): Window pooling method. Default: none. 
        diagswin_level (int): Number of diagswin levels. Default: 1.
        diagswin_window (int): region size at each diagswin level. Default: 1.
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, expand_size, expand_layer="all",
                 ...
                 diagswin_level=1, diagswin_window=1, use_conv_embed=False, use_shift=False, use_pre_norm=False,
        ...
        self.blocks = nn.ModuleList([
            DiagSWinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 num_heads=num_heads,
                                 window_size=window_size,
                                 # shift_size=(0 if (i % 2 == 0) else window_size // 2) if use_shift else 0,
                                 use_shift=use_shift if(i%2==0) else True,
                                 ...
                                 diagswin_level=diagswin_level,
                                 diagswin_window=diagswin_window,
...
class DiagSWinTransformer(nn.Module):
    r""" DiagSWin Transformer: DIagSWin Self-attention for Local-Global Interactions in Vision Transformer
    ...
    Args:
        ...
        depths (tuple(int)): Depth of each DiagSWin Transformer layer.
        ...
        diagswin_stages (list): Which stages to perform diagswin attention. Default: [0, 1, 2, 3], means all stages
        diagswin_levels (list): How many diagswin levels at all stages. Note that this excludes the finest-grain level. Default: [1, 1, 1, 1]
        diagswin_windows (list): The diagswin window size at all stages. Default: [7, 5, 3, 1]
    ...
    def __init__(self, 
                ...
                use_shift=False,
                diagswin_stages=[0, 1, 2, 3],
                diagswin_levels=[1, 1, 1, 1],
                diagswin_windows=[7, 5, 3, 1],
                diagswin_pool="fc",
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               norm_layer=norm_layer, 
                               pool_method=diagswin_pool if i_layer in diagswin_stages else "none",
                               downsample=PatchEmbed if (i_layer < self.num_layers - 1) else None,
                               diagswin_level=diagswin_levels[i_layer],
                               diagswin_window=diagswin_windows[i_layer],
...
if __name__ == '__main__':
    ...
    model = DiagSWinTransformer(img_size=img_size, embed_dim=96, depths=[2,2,6,2], drop_path_rate=0.2,
        diagswin_levels=[3,3,3,3], expand_sizes=[3,3,3,3], expand_layer="all",
        num_heads=[3,6,12,24],
        diagswin_windows=[7,5,3,1],
        window_size=7,
        use_conv_embed=True,
